        ...
        Promise.all(
            connections.map(async (connection) => {
                if (connection.driver.options.type === "spanner") {
                    ...
                    await createSpannerInstanceIfNotExists(option.instanceId!)
                    await createSpannerDatabaseIfNotExists(option.databaseId!)
                }
...
async function createSpannerDatabaseIfNotExists(
    ...
    return new Promise((resolve, reject) => {
        ...
        const options = {
            hostname: "localhost",
            port: 9020,
            path: "/v1/projects/test-project/instances/test-instance/databases",
            method: "POST",
            headers: {
...
async function createSpannerInstanceIfNotExists(
    ...
    return new Promise((resolve, reject) => {
        ...
        const options = {
            hostname: "localhost",
            port: 9020,
            path: "/v1/projects/test-project/instances",
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Content-Length": Buffer.byteLength(data),
